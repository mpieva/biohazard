...
gather refs = filterStream (\b -> not (br_isPaired b) || br_isFirstMate b) =$
              -- filterStreamM (const . liftIO $ (< (0.01 :: Double)) <$> randomIO) =$
              -- takeStream 10000 =$
              progress "reading " (hPutStr stderr) refs =$
              mapStream (fromTags "XI" "YI" &&& fromTags "XJ" "YJ") =$ stream2vector
...
-- | Matches one index against a list of indices.
matchAll :: Index -> U.Vector Index -> U.Vector Word64
matchAll x = U.map (match x)

-- | A mixture description is one probability for each combination of p7
-- and p5 index.  They should sum to one.
type Mix = U.Vector Double

-- | Computing the naively assumed mix when nothing is known:  uniform
-- distribution.
unknownMix :: (Int,Int) -> Mix
unknownMix (n7,n5) = U.replicate total (recip $ fromIntegral total)
  where
    !total = n7*n5
...
naiveMix (n7,n5) ps = U.replicate total (0.01 / fromIntegral (total-known))
                 U.// [ (n5*i7+i5, 0.99 / fromIntegral known) | (i7,i5) <- ps ]
  where
    !total = n7*n5
    !known = length ps
...
unmix1 :: U.Vector Index -> U.Vector Index -> Mix -> (Index, Index) -> Mix
unmix1 p7 p5 prior (x,y) =
  let !m7 = U.map phredPow $ matchAll x p7
      !m5 = U.map phredPow $ matchAll y p5

  -- *sigh*, concatMap doesn't fuse.  Gotta do it manually.  :(
  in U.create $ do v <- UM.new (U.length m7 * U.length m5)
                   let loop !i !j !k !acc
                           | i == U.length m7 = loop2 (recip acc) 0
                           | j == U.length m5 = loop (i+1) 0 k acc
                           | otherwise = let !p = prior U.! k * m7 U.! i * m5 U.! j
                                         in UM.write v k p >> loop i (j+1) (k+1) (acc + p)

                       loop2 !tot !k
                           | k == U.length m7 * U.length m5 = return v
                           | otherwise = UM.read v k >>= UM.write v k . (*) tot >> loop2 tot (k+1)
...
iterEM :: U.Vector (Index, Index) -> U.Vector Index -> U.Vector Index -> Mix -> Mix
iterEM pairs p7 p5 prior = U.foldl' (\acc xy -> U.zipWith (+) acc $ unmix1 p7 p5 prior xy)
                                    (U.replicate (U.length prior) 0) pairs
...
main = do
    ...
    let initial = unknownMix (U.length p7is, U.length p5is)
    mapM_ (inspect p7ns p5ns) $ iterate (iterEM ixvec p7is p5is) initial

inspect :: V.Vector T.Text -> V.Vector T.Text -> Mix -> IO ()
inspect n7 n5 mix = do
    getClockTime >>= print
    v <- U.thaw $ U.indexed mix
    V.partialSortBy (\(_,a) (_,b) -> compare b a) v 20
    v' <- U.unsafeFreeze $ UM.take 20 v
    U.forM_ v' $ \(i,n) -> do
       let (i7, i5) = i `quotRem` V.length n5
       T.print "{}, {}: {}\n"
            ( T.left 7 ' ' $ n7 V.! i7
            , T.left 7 ' ' $ n5 V.! i5
