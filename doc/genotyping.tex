...
\idea{The Nielsen method to infer IBD (or inbred) tracts assumes HWE and
relies on an estimate of allele frequencies per site.  That's of course
ridiculous if only one individual is available, and I think the method
should fail on a single individual.  (They must be using multiple
individuals from a homogenous population.  Confirm?)  However, we
already estimate allele frequencies as a side effect.  Given these, the
Nielsen method could be adapted!  It could actually be simplified, the
frequencies are constant, only the HMM parameters need to be inferred.}
